...
public class BloomFilter {
    ...
    // as error rate, 10/100 = 0.1  作为错误率，10/100=0.1
    private int f = 10;
    private int n = 128;

    // hash function num, by calculation.   哈希函数num，通过计算
    private int k;
    // bit count, by calculation.   位计数，通过计算
    private int m;

    /**
     * Create bloom filter by error rate and mapping num.   按错误率和映射数创建bloom筛选器。
     *
     * @param f error rate
    ...
    /**
     * Calculate bit positions of {@code str}.  计算 {@code str} 的位位置。
     * <p>
     * See "Less Hashing, Same Performance: Building a Better Bloom Filter" by Adam Kirsch and Michael
     * Mitzenmacher.
     * 请参阅Adam Kirsch和Michael Mitzenmacher的“更少的散列，相同的性能：构建更好的Bloom过滤器”
     * </p>
     */
    public int[] calcBitPositions(String str) {
        ...
        for (int i = 1; i <= this.k; i++) {
            ...
            // Flip all the bits if it's negative (guaranteed positive number)  如果是负数，则翻转所有位（保证正数）
            if (combinedHash < 0) {
