...
public class MessageStoreConfig {
    ...
    //The root directory in which the log data is kept    保存日志数据的根目录
    @ImportantField
    private String storePathRootDir = System.getProperty("user.home") + File.separator + "store";

    //The directory in which the commitlog is kept      保存提交日志的目录
    @ImportantField
    ...
    // CommitLog file size,default is 1G        CommitLog 文件大小，默认为 1G
    private int mappedFileSizeCommitLog = 1024 * 1024 * 1024;
    // ConsumeQueue file size,default is 30W        消费队列文件大小，默认为 30W
    private int mappedFileSizeConsumeQueue = 300000 * ConsumeQueue.CQ_STORE_UNIT_SIZE;
    // enable consume queue ext         启用消费队列分机
    private boolean enableConsumeQueueExt = false;
    // ConsumeQueue extend file size, 48M       ConsumeQueue 扩展文件大小，48M
    private int mappedFileSizeConsumeQueueExt = 48 * 1024 * 1024;
    // Bit count of filter bit map.     过滤器位图的位数
    // this will be set by pipe of calculate filter bit map.        这将由计算过滤器位图的管道设置。
    private int bitMapLengthConsumeQueueExt = 64;

    // CommitLog flush interval     CommitLog 刷新间隔
    // flush data to disk       将数据刷新到磁盘
    @ImportantField
    private int flushIntervalCommitLog = 500;

    // Only used if TransientStorePool enabled      仅在启用 TransientStorePool 时使用
    // flush data to FileChannel        将数据刷新到文件通道
    @ImportantField
    private int commitIntervalCommitLog = 200;

    /**
     * introduced since 4.0.x. Determine whether to use mutex reentrantLock when putting message.<br/>
     * 从 4.0.x 开始引入。判断是否在放置消息时使用互斥重入锁。
     */
    private boolean useReentrantLockWhenPutMessage = true;

    // Whether schedule flush           是否定时刷新
    @ImportantField
    private boolean flushCommitLogTimed = true;
    // ConsumeQueue flush interval      消费队列刷新间隔
    private int flushIntervalConsumeQueue = 1000;
    // Resource reclaim interval        资源回收间隔
    private int cleanResourceInterval = 10000;
    // CommitLog removal interval       CommitLog 删除间隔
    private int deleteCommitLogFilesInterval = 100;
    // ConsumeQueue removal interval        消费队列移除间隔
    private int deleteConsumeQueueFilesInterval = 100;
    private int destroyMapedFileIntervalForcibly = 1000 * 120;
    private int redeleteHangedFileInterval = 1000 * 120;
    // When to delete,default is at 4 am        何时删除，默认为凌晨 4 点
    @ImportantField
    private String deleteWhen = "04";
    private int diskMaxUsedSpaceRatio = 75;
    // The number of hours to keep a log file before deleting it (in hours)     在删除之前保留日志文件的小时数（以小时为单位）
    @ImportantField
    private int fileReservedTime = 72;
    // Flow control for ConsumeQueue        ConsumeQueue 的流量控制
    private int putMsgIndexHightWater = 600000;
    // The maximum size of message,default is 4M        消息的最大大小，默认为 4M
    private int maxMessageSize = 1024 * 1024 * 4;
    // Whether check the CRC32 of the records consumed.        是否检查消费记录的CRC32。
    // This ensures no on-the-wire or on-disk corruption to the messages occurred.  这可确保不会对发生的消息进行在线或磁盘损坏。
    // This check adds some overhead,so it may be disabled in cases seeking extreme performance.    此检查会增加一些开销，因此在寻求极端性能的情况下可能会禁用它。
    private boolean checkCRCOnRecover = true;
    // How many pages are to be flushed when flush CommitLog    刷新 CommitLog 时要刷新多少页
    private int flushCommitLogLeastPages = 4;
    // How many pages are to be committed when commit data to file  提交数据到文件时要提交多少页
    private int commitCommitLogLeastPages = 4;
    // Flush page size when the disk in warming state   磁盘处于预热状态时刷新页面大小
     ...
     * ASYNC_FLUSH
     * 仅当transientStorePoolEnable 为 true 且 FlushDiskType 为 ASYNC_FLUSH 时才启用瞬态 commitLog 存储池
     *
